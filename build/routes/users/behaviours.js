...
router.get('/add-behaviour/:skill', ensureAuthenticated, function (req, res, next) {
    ...
    connection.get().query('SELECT skill_id,skill_title FROM skills', function (err, skillResults) {
        ...
        if (err) {
            ...
            connection.get().query('SELECT skills.skill_id,skills.skill_title,behaviour_id,behaviours.behaviour_desc FROM skills INNER JOIN behaviours ON skills.skill_id = behaviours.skill_id ' + 'WHERE skills.skill_id = ?', req.params.skill, function (err, behaviourResults) {
                //If an error is thrown
                if (err) {
                    returnObj['message'] = 'Our database servers maybe down. Please try again.';
                    //Render the page wth error messages
...
router.post('/add-behaviour/:skill/:canRemove', ensureAuthenticated, function (req, res, next) {
    ...
    async.series([filterPostData, deleteSkills, deleteBehaviours, updateSkill, updateBehaviour, redirectUser], function (err, results) {});
